...
The order is uploading the file temperature_optimisation (F530917_25WSA032_Coursework_V1103\arduino\Arduino Task\temperature_optimisation) into the arduino. I had better results using the Arduino IDE to upload the code to the arduino, but if there is no problems with the VScode setup on your end then hopefully it should be fine.
...
Now go back to VScode, open capture_serial.py located at (F530917_25WSA032_Coursework_V1103\arduino\Arduino Task\test). run the file as you would a standard python file, click run or F5. It should do the same thing as the Arduino code where it should wait a little while (approx 1min), except the difference is the values are now stored in a CSV. The CSV file will be generated automatically. The end should say "Saved to temperature_data.csv"

Once it has said the values have been stored, run the analyse_temperature.py file (F530917_25WSA032_Coursework_V1103\arduino\Arduino Task\test), and this should generate 5 graphs according to the brief. This will also save those graphs into temperature_analysis_live.png, which can be further analysed (File location: F530917_25WSA032_Coursework_V1103).
